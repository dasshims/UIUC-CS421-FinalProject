...
getErrorReport names program = do
  ...
  where
--  Change of exception reporting in GHC 6.6 vs. GHC 6.8
  pattern_6_6 = ".*(\\*\\*\\* Exception: .*)"
  pattern_6_8 = ".*(" ++ lhs program ++ ": .*)"
  getexc s = case matchRegexAll (mkRegex pattern_6_8) s `mplus`
                  matchRegexAll (mkRegex pattern_6_6) s of
               Nothing           -> []
...
runtest out lib program = do
  ...
  run (ghc [srcdir lib,lhs program] 
      ...
      (\_ -> do
         let
           -- runtime errors are printed in stdout by GHC 6.6
           -- and stderr by GHC 6.8
           generated = [out program               -- GHC 6.6
                       ,compilestats program lib] -- GHC 6.8
