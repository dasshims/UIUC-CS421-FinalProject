...
{-# LINE 110 "NOW.lhs" #-}
  ...
  -- AG: added type signatures for eval test here due to the non-openness 
  toSpine (TreeR a w :> Leaf x) = Con leaf :$ (a :> x)
  toSpine (TreeR a w :> Fork l r) = Con fork :$ (TreeR a w :> l) :$ (TreeR a w :> r)
  toSpine (TreeR a w :> WithWeight t f) = Con withweight :$ (TreeR a w :> t) :$ (w :> f)
...
{-# LINE 1233 "NOW.lhs" #-}
  ...
  nil     =  Descr { constr = [],     name = "Nil",    arity = 0,  fixity = Prefix 10,  order = (0, 2) }
  cons    =  Descr { constr = (:),  name = "Cons",   arity = 2,  fixity = Prefix 10,  order = (1, 2) }
  myin    =  Descr { constr = In,      name = "In",     arity = 1,  fixity = Prefix 10,  order = (0, 1) }
{-# LINE 1257 "NOW.lhs" #-}
  zero    =  Descr { constr = Zero,    name = "Zero" }
  succ    =  Descr { constr = Succ,    name = "Succ" }
  num     =  Descr { constr = Num,     name = "Num" }
  plus    =  Descr { constr = Plus,    name = "Plus" }
{-# LINE 1277 "NOW.lhs" #-}
...
{-# LINE 3464 "NOW.lhs" #-}
  con c s = Con (Descr { constr = c, name = s})
{-# LINE 3519 "NOW.lhs" #-}
...
{-# LINE 3559 "NOW.lhs" #-}
  eq   =  Descr { constr = Eq,    name = "Eq" }
  ifV  =  Descr { constr = If,    name = "If" }
{-# LINE 3577 "NOW.lhs" #-}
