...
--       dataCxtTypes = nub (typeParams ++ types)
       dataCxtTypes = nub (typeParams ++
                           ...
                           --      Data ctx [a] where ...
                           --
                           -- Which causes looping, for an example see
                           -- BugBinTree.lhs.
                           -- On the other hand this fix is ad-hoc, it does
                           -- not work for datatypes such as Company and
                           -- Perfect.
                           -- --Alexey
                           map VarT (concatMap freeVars types))
       satCxtTypes = nub (myType : types)
       context = cxt (map dataCxt dataCxtTypes ++ map satCxt satCxtTypes)
#endif

-- ironic that this is not implemented with GP :)
-- pasted from http://www.cs.chalmers.se/~ulfn/code/genericth.tar.gz/Common/THUtil.hs
freeVars :: Type -> [Name]
freeVars (VarT x)	    = [x]
